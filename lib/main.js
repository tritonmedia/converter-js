...
module.exports = async (config, queue) => {
  ...
  const processor = async (container, done) => {
    ...
    emitter.on('finished', () => {
      ...
      let jobIndex = activeJobs.indexOf(fileId)
  ...
  return () => {
    ...
    for (let activeJob of activeJobs) {
      const e = new Error('Converter was terminated before it could finish.')
      e.code = 'ERREARLYEXIT'
      EmitterTable[activeJob].emit('error', e)
    }
  }
}
