...
module.exports = config => {
  ...
  for (const key of Object.keys(audioObject)) {
    if (key === '' || typeof key === 'undefined' || key === null) delete audioObject[key]
  }

  for (const key of Object.keys(videoObject)) {
  ...
  return async (job, filePath) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      child.info('writing file', filename, output)
      ...
      const progressReporter = setInterval(() => {
        child.info(`progress ${progress} eta: ${eta}`)
      }, 10000)
      ...
      conv.spawn(convObject)
        ...
        .on('end', () => {
          ...
          endCatch = setTimeout(() => {
            if (!completed) {
              child.error('failed to invoke handbrake')
              return reject(new Error('Failed to convert. See log.'))
            }
          }, 5000)

          child.debug('end fired')
